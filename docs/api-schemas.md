...
`GET /v1/models` includes gateway-owned `meta` fields for the configured llama.cpp runtime budget:
...
These values are the router preset settings generated by the gateway for catalog models. They are intended for client-side budgeting and diagnostics; llama.cpp router mode still owns actual load/unload and scheduling.

If llama.cpp returns `Context size has been exceeded.`, the gateway normalizes it to an OpenAI-style `400 invalid_request_error` with code `context_length_exceeded`. Long non-streaming requests can also hit the gateway response-header timeout; tune `GATEWAY_PROXY_RESPONSE_HEADER_TIMEOUT_SECONDS` or use streaming for long-running generations.
